<x-app-layout>
    <div class="container max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
        <h1 class="mb-8 text-3xl font-bold text-gray-800">Créer un nouveau produit</h1>
        ...
        <form action="{{ route('products.store') }}" method="POST"
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <x-input-select-dynamic name="categoryId" :options="$categories" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" />
                    <x-input-error :messages="$errors->get('categoryId')" />
                </div>

                <div>
                    <div class="flex items-center gap-2 mb-2">
                        <x-input-label name="ingredients" value="Ingredients" class="text-sm font-medium text-gray-700" />
    ...
    <script>
        function addIngredient() {
            ...
            newIngredientDiv.className = 'flex items-center px-3 py-1.5 bg-blue-100 text-blue-800 rounded-full';
            newIngredientDiv.innerHTML = `
                <span class="text-sm">${selectedIngredient.text}</span>
                <button type="button" class="ml-2 text-blue-600 transition-colors hover:text-red-500" onclick="removeIngredient(${selectedIngredientId})">×</button>
